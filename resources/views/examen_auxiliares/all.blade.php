                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <ul class="table-controls">
                                            ...
                                            <li><span data-toggle="modal" data-target="#labEditModal"><a href="javascript:void(0);" class="bs-tooltip" data-toggle="tooltip" data-placement="top" title="" data-original-title="Editar" onclick="$('#lab_titulo').text('{{ addslashes($exam->titulo) }}'); $('#lab_edit_titulo').val('{{ old('lab_edit_titulo', addslashes($exam->titulo)) }}'); $('#lab_edit_descripcion').val(`{{ old('lab_edit_descripcion', addslashes($exam->descripcion)) }}`); $('#lab_edit_form').prop('action', '{{ route('examen_auxiliares.update', $exam->id) }}'); $('#lab_edit_file_url').text('{{ addslashes(substr($exam->url, 8)) }}'); $('#lab_edit_file_download').attr('href', '{{ Storage::url(addslashes($exam->url)) }}');if('{{ addslashes($exam->url) }}' == ''){ $('#lab_replace_file').hide();$('#lab_put_file').show(); }else{ $('#lab_replace_file').show();$('#lab_put_file').hide(); } "><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="p-1 mb-1 feather feather-edit-2 br-6"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg></a></li>
