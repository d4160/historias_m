<x-layouts.admin title="Tratamientos" bodyTitle="Tratamientos">
    <x-slot name="styles">
                ...
                <div class="widget-content widget-content-area">
                    <div class="mb-4 table-responsive">
                        ...
                        <div class="page-header">
                            <label class="ml-3" style="font-weight: bold; color: black; font-size: 17px; margin-bottom: 31px;">Lista de Tratamientos</label>

                            <button type="button" data-toggle="modal" data-target="#examModal"
                            class="ml-3 btn btn-success" style="margin-bottom: 10px;" onclick="$('#exam_modal_historia_id').val({{ $historia->id }}); $('#formExam').attr('action', '{{ route('tratamientos.store') }}'); $('#exam-title').text('Tratamiento');">Nuevo Tratamiento</button>
                        </div>
                        ...
                        <table id="exam_table" class="table style-3 table-hover">
                            ...
                            <tbody>
                                ...
                                @foreach ($historia->tratamientos as $trat)
                                <tr>
                                        ...
                                        <ul class="table-controls">
                                            ...
                                            <li><form id="delete_exam_{{ $trat->id }}_form" method="POST" action="{{ route('tratamientos.destroy', $trat->id) }}" style="display: inline-block">
                                                ...
                                                <a href="javascript:void(0);" class="bs-tooltip exam_remove confirm"
                                                                        {{-- form_id="delete_exam_{{ $trat->id }}_form"
                                                                        exam_aux_title="{{ addslashes($trat->tratamiento) }}" --}}
                                                                        data-container="body" data-html="true" data-toggle="tooltip" data-placement="top" title="" data-original-title="Eliminar" onclick="ConfirmDeleteTrat('delete_exam_{{ $trat->id }}_form', '{{ addslashes($trat->tratamiento) }}')">
    ...
    <x-slot name="scripts">
        ...
        <script>
            ...
            //         swal({
            //             title: `¿Está seguro de eliminar el tratamiento '${exam_aux_title}' del registro del sistema?`,
            //             type: 'warning',
            ...
            function ConfirmDeleteTrat(form_id, exam_aux_title) {
                swal({
                    title: `¿Está seguro de eliminar el tratamiento '${exam_aux_title}' del registro del sistema?`,
                    type: 'warning',
                    showCancelButton: true,
